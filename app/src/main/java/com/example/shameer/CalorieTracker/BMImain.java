...
import static com.example.shameer.CalorieTracker.bmicalc.BMI_CATEGORY_HEALTHY;
import static com.example.shameer.CalorieTracker.bmicalc.BMI_CATEGORY_OBESE;
import static com.example.shameer.CalorieTracker.bmicalc.BMI_CATEGORY_OVERWEIGHT;
import static com.example.shameer.CalorieTracker.bmicalc.BMI_CATEGORY_UNDERWEIGHT;
...
public class BMImain extends AppCompatActivity {
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            button=findViewById(R.id.backbut);

            button.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent intent = new Intent(BMImain.this,MainActivity.class);
                ...
                public void onClick(View view) {
                    if (inMetricUnits) {
                        if (weightKgEditText.length() == 0 || heightCmEditText.length() == 0) {
                            Toast.makeText(BMImain.this, "Populate Weight and Height to Calculate BMI", Toast.LENGTH_SHORT).show();
                        } else {
                            ...
                            double bmi = bmicalc.getInstance().calculateBMIMetric(heightInCms, weightInKgs);
                            displayBMI(bmi);
                        }
                    } else {
                        if (weightLbsEditText.length() == 0 || heightFtEditText.length() == 0 || heightInEditText.length() == 0) {
                            ...
                            double bmi = bmicalc.getInstance().calculateBMIImperial(heightFeet, heightInches, weightLbs);
                            displayBMI(bmi);
                        }
                    }
                }
        ...
        private void displayBMI(double bmi) {
            ...
            String bmiCategory = bmicalc.getInstance().classifyBMI(bmi);
